...
feature -- Test routines
	...
	test_creation
		...
		do
			...
			assert ("field_zero1", m [1,1] = 1.0)
			assert ("field_zero2", m [1,2] = 2.0)
		end
	...
	test_is_symmetric
		...
		do
			...
			m := al.array_matrix (<< << 1.0 >> >>)
			assert ("symmetric2", m.is_symmetric)
			m := al.array_matrix (<< << 2.0, 2.0 >>, << 2.0, 2.0 >> >>)
	...
	test_put
		...
		local
			m: AL_MATRIX
		do
			m := al.array_matrix (<< << 1.0, 1.0 >>, << 1.0, 1.0 >> >>)
			m.put (-1.0, 1, 1)
			m.put (1.0, 1, 1)
			m.put (2.0, 2, 1)
			m [1, 2] := 3.0
	...
	test_fill
		...
		local
			m: AL_MATRIX
		do
			m := al.array_matrix (<< << 0,0,0 >>, <<0,0,0>>, <<0,0,0>> >>)
			m.fill (11.00)
